...
| Keeper scorer weight | keepers never finish open-play chances (weight 0); at set-piece headers their weight is divided by 8, keeping the famous last-corner keeper goal possible but rare | `internal/engine` `pickScorerForChance`, `gkSetPieceScorerDivisor` | [12 §5](12-match-model.md) |
| Squad selection shape | XI takes the tactical plan's formation bands (first band defenders, last forwards, midfield between); unset/invalid shapes read as 4-4-2; depleted bands fall back to best leftovers; bench seat 1 reserved for a spare keeper | `internal/engine` `formationBands`, `selectSquad`, `benchSize` | [12 §6](12-match-model.md) |
| Red-card suspension | 1 club fixture per red card (straight or second yellow); served as the club's fixtures complete; banned players excluded from selection | `internal/engine` `suspensionMatchesRed`, `applySuspensions`, `selectSquad` | [12 §7](12-match-model.md) |
| Match commentary context | goals from 85' narrate as late drama; opener/equalizer calls replace the pattern line; a scorer's 3rd goal headlines as a hat-trick; leveling/leading after a 2-goal deficit narrates as a comeback; re-taking the lead within 5' of conceding reads as a response; a 4th goal at a 3-goal margin reads as a rout (presentation only, no RNG) | `internal/engine` `lateDramaMinute`, `hatTrickGoals`, `comebackDeficitMin`, `responseWindowMinutes`, `routGoalsMin`, `routMarginMin`, `goalContextCommentaryKey` | [12 §7](12-match-model.md) |
| Match quiet-beat context | close games from 75' prefer tension lines; ≥3-goal margins from 60' prefer cruise lines; identical single legacy-bound draw either way | `internal/engine` `tensionQuietMinute`, `cruiseQuietMinute`, `cruiseQuietMargin`, `quietBeatKey` | [12 §7](12-match-model.md) |
...
| Condition model | drain 22/match (floor 5) · recover 14/day · sharpness +8/match | `internal/engine` `conditionDrainPlay`/`conditionFloorPlay`/`conditionRecoveryPerDay`/`sharpnessGainPlay` | [02 §2.3](02-game-design.md) |
...
| Quiet commentary pool | 15 variants, no repeats within a match until exhausted; retains one `IntN(15)` draw per quiet moment | `internal/engine` `quietCommentaryKeys` / `pickUnusedCommentaryKey` | [02 §4](02-game-design.md) |
