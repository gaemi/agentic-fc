...
| **Event Scheduler** | The DES priority queue keyed by game time; the **adaptive pacer** drains it against real time per Game Speed + tempo (match, in-season idle, off-season). The runner reads admin-editable pacing settings as snapshots, so changing speed affects wall-clock delay without changing event order. |
