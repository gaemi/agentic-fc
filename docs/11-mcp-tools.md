...
**Params**: none.
**Returns**: league position & last 3 results; next fixture; urgent items (injuries, suspensions, expiring contracts, active offers awaiting Manager decision, board mood flag, ultimatum if any); 5 newest compact headline refs `{id, game_time, category, headline{key,params,text}, article{source,title,deck}, refs}`; window status. Headline params omit matchday fixture/result/table/story detail and article previews omit `body`; call `get_news` to open the full clipping. **Unemployed variant**: reputation summary, open vacancies list, active job approaches, compact headline refs.
...
**Returns**: condensed narrative items `{id, game_time, category, headline{key,params,text}, article{source,title,deck,body}, refs[]}` + next cursor. `headline` remains the compact machine-readable news fact; `article` is the human press clipping used by MCP Apps cards, rendered only from the same masked params. Repeated injury and matchday events select a deterministic editorial angle from `id`; matchday article titles, decks, and bodies share that angle, and the same item keeps the same prose on reread and in Console/MCP Apps without consuming simulation RNG. Manager decisions appear here with their Mindset version (FR-16e).
...
**Returns (finished)**: score, scorers, cards, stats table (`home_shots`, `away_shots`, `match_patterns[]`, `shot_quality[]`, `aerial_duels[]`, `aerial_wins[]`, `press_turnovers[]`, `set_piece_threat[]`, `tactical_tilt[]`), lineups, substitutions, player ratings, key-moment commentary log (condensed). Each `match_patterns` row carries `side: HOME | AWAY`, `pattern`, localized English `label`, and `count`; each `shot_quality` row likewise carries its side. Results completed before side tracking use `UNKNOWN` rather than guessing attribution. If a live match straddles an upgrade, new moments are attributed while the older aggregate remainder stays as `UNKNOWN`. Pattern rows are HOME then AWAY then UNKNOWN, with each side's most frequent patterns first. Quality rows are HOME then AWAY, each HIGH/MEDIUM/LOW, followed by any UNKNOWN remainder.
**Returns (live)**: current score/clock, last ~20 commentary lines + cursor, stats snapshot (`home_shots`, `away_shots`, `match_patterns[]`, public diagnostic rows), own-team state (current derived condition for each on-pitch player, bookings), Manager's in-match adjustments so far. A starter's live condition drains across the match; a substitute drains from the minute they entered. `match_patterns[]` is a public tactical read such as crosses, cutbacks, or counters — useful for play, not a formula dump. Diagnostics are observed match facts: quality bands, aerial counts, press turnovers, set-piece threat, and tilt families. Polling a live match costs per call — watching closely is a real Focus decision for *other* matches, cheap for your own.
