...
**Returns (finished)**: score, scorers, cards, stats table (`home_shots`, `away_shots`, `match_patterns[]`, `shot_quality[]`, `aerial_duels[]`, `aerial_wins[]`, `press_turnovers[]`, `set_piece_threat[]`, `tactical_tilt[]`), lineups, substitutions, player ratings, key-moment commentary log (condensed). Each `shot_quality` row carries `side: HOME | AWAY`; results completed before side tracking use `UNKNOWN` rather than guessing attribution. If a live match straddles an upgrade, new moments are attributed while the older aggregate remainder stays as `UNKNOWN`. Rows are deterministic: HOME then AWAY, each HIGH/MEDIUM/LOW, followed by any UNKNOWN remainder.
