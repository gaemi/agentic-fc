...
the addresses that were actually bound, so `-console-addr 127.0.0.1:0` is a
safe way to launch without picking a port first; point the console's `-server`
flag at the printed address.
