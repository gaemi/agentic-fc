...
**story-of-the-match report** (a result frame, at most one diagnostics edge
such as pressing/aerial/set-piece/chance-quality dominance or the winner's
chance-pattern identity, and at most one ledger story beat such as a
hat-trick, a two-goal comeback, or a late winner — rendered server-side from
public facts only, variants rotated deterministically by fixture ID), and the
...
- **Disconnected state:** before the first successful catalog fetch, the Console cannot show server-rendered text, so a small built-in English fallback set covers the app chrome, and a Console that cannot reach the server at all swaps the tab body for a guidance panel: the server URL it tried, whether the daemon is running, the `-server` override, and the live retry error. Normal polling doubles as reconnection — once the daemon answers, the world view replaces the panel without a restart. A Console that already holds a catalog keeps the ordinary tab body on transient errors and shows them in the status line (server-localized, since the catalog is present).
