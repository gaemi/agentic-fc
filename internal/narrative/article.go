...
func ArticleTemplateKey(section, class string, newsID int64) string {
	...
	variant := int(mix64(uint64(newsID))%uint64(count)) + 1
	if variant == 1 {
		return base
	}
...
// mix64 is the splitmix64 finalizer: it turns a persisted ID into a
// well-scattered value so presentation variants keyed off sequential news IDs
// do not stripe, while consuming no simulation RNG.
